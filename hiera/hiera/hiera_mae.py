...
class MaskedAutoencoderHiera(Hiera):
    ...
    def get_pixel_label_2d(
        ...
        size = self.pred_stride                                         # pred多少倍下采样
        label = input_img.unfold(1, size, size).unfold(2, size, size)
        label = label.flatten(1, 2).flatten(2)
        label = label[mask]                                             # 筛选mask units
        if norm:                                                        # layernorm
    ...
    def forward_encoder(
        ...
        if mask is None:
            mask = self.get_random_mask(x, mask_ratio)  # [B, #MUs_all], 1 is for keep, 0 is for real mask

        # Get multi-scale representations from encoder
        _, intermediates = super().forward(x, mask, return_intermediates=True)      # 使用父类的forwar获取每个stage特征图, mask在父类中用于过滤patch
        # Resolution unchanged after q_pool stages, so skip those features
        intermediates = intermediates[: self.q_pool] + intermediates[-1:]           # 提取分辨率变换阶段的最后一张特征图

        # Multi-scale fusion
        x = 0.0
        for head, interm_x in zip(self.multi_scale_fusion_heads, intermediates):    # 特征降采样融合, multiscale
    ...
    def forward_decoder(
        ...
        x_dec[mask] = x.flatten()                               # 将没有mask的latent补全
        x_dec = ~mask * mask_tokens + mask * x_dec              # 将mask掉的patch补全params

        # 恢复到原来图片对应的path顺序
        x = undo_windowing(
            x_dec,
            self.tokens_spatial_shape_final,                    # 56 --> 28 --> 14 , 3 stages
            self.mask_unit_spatial_shape_final,
        )                                                    
        mask = undo_windowing(
            mask[..., 0:1],
        ...
        # Add pos embed
        x = x + self.decoder_pos_embed                          # 绝对位置编码
    ...
    def forward_loss(
        ...
        """
        Note: in mask, 0 is *visible*, 1 is *masked*, 1 is for mask_units

        x: e.g. [B, 3, H, W], raw image input
        ...
        pred = pred[mask]                   # 筛选对应的mask_units的preds
        loss = (pred - label) ** 2          # 计算回归损失

        return loss.mean(), pred, label     # all for mask_unit
    ...
    def forward(
        ...
        latent, mask = self.forward_encoder(x, mask_ratio, mask=mask)   # mask for mask_unit for unroll
        pred, pred_mask = self.forward_decoder(                         # pred_mask for no mask_unit for reroll
            latent, mask
