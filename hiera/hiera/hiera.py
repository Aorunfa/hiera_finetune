...
class MaskUnitAttention(nn.Module):
    ...
    def __init__(
        ...
        self.heads = heads                      # 划分多少个头
        self.q_stride = q_stride                # q的maxpool步长

        self.head_dim = dim_out // heads        # 每个头的特征维度
        self.scale = (self.head_dim) ** -0.5    # q*k^T放缩参数

        self.qkv = nn.Linear(dim, 3 * dim_out)  # qkv的投影矩阵合并在一起
        self.proj = nn.Linear(dim_out, dim_out) # o 的投影矩阵

        self.window_size = window_size          # mask的patch长度 e.g 8*8
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        )                                        # 计算mask unit window的个数， 每个window内的patch进行注意力，局部注意力机制
        ...
        qkv = (
            ...
            .permute(3, 0, 4, 2, 1, 5)          # shape(3, B, head, num_window, patch_num, dim_out), patch_num for mask unit num eg. 8*8
        )
        ...
        if self.q_stride > 1:
            ...
            q = (
                q.view(B, self.heads, num_windows, self.q_stride, -1, self.head_dim) # patch_num 降低采样
                .max(dim=3)
                .values
            )
                                                 # 分头分窗口进行注意力机制，多头局部注意力机制，局部大小为maskunit的patch区域
        ...
        x = x.transpose(1, 3).reshape(B, -1, self.dim_out) # 按照window patch进行展平
        x = self.proj(x)
        return x
...
class HieraBlock(nn.Module):
    def __init__(
        ...
        dim_out: int,
        heads: int,
        mlp_ratio: float = 4.0,     # 隐藏层维度
        drop_path: float = 0.0, 
        norm_layer: nn.Module = nn.LayerNorm,
        act_layer: nn.Module = nn.GELU,
        q_stride: int = 1,
        window_size: int = 0,       #### 什么作用
        use_mask_unit_attn: bool = False,
    ):
        super().__init__()
        ...
        if dim != dim_out:
            self.proj = nn.Linear(dim, dim_out)                         # 下采样投影矩阵，扩充特征空间，再由do_pool减小分辨率

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Attention + Q Pooling
        x_norm = self.norm1(x)
        if self.dim != self.dim_out:
            x = do_pool(self.proj(x_norm), stride=self.attn.q_stride)   # stage end下采样
...
class Hiera(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        stages: Tuple[int, ...] = (2, 3, 16, 3),                    # 每个stage堆积几个vit block
        q_pool: int = 3,  # number of q_pool stages
        q_stride: Tuple[int, ...] = (2, 2),                         # 对q对应的patch进行池化的kernal大小
        mask_unit_size: Tuple[int, ...] = (8, 8),  # must divide q_stride ** (#stages-1)， mask掉的patch方阵？ ######
        # mask_unit_attn: which stages use mask unit attention?
        mask_unit_attn: Tuple[bool, ...] = (True, True, False, False),
        dim_mul: float = 2.0,
        head_mul: float = 2.0,
        patch_kernel: Tuple[int, ...] = (7, 7),
        patch_stride: Tuple[int, ...] = (4, 4),                      # patch embedding conv步长 
        ...
        self.stage_ends = [sum(stages[:i]) - 1 for i in range(1, len(stages) + 1)]             # 每个stage结束的idx
        ...
        self.unroll = Unroll(
            input_size, patch_stride, [q_stride] * len(self.stage_ends[:-1]) # q_stride: max_pool kernal;  self.stage_ends 下采样stage
        )
        self.reroll = Reroll(
            input_size,
            patch_stride,
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)] # res path dropout, 均匀增长
        ...
        for i in range(depth):
            ...
            use_mask_unit_attn = mask_unit_attn[cur_stage]   # 是否使用局部注意力
    ...
    def get_random_mask(self, x: torch.Tensor, mask_ratio: float) -> torch.Tensor:
        ...
        num_windows = math.prod(self.mask_spatial_shape)  # num_mask_units , 连续patch可以划分多少的mask unit
        ...
        mask[:, :len_keep] = 1                                   # 保证每个batch mask的比例相同
        # Unshuffle to get the binary mask
        mask = torch.gather(mask, dim=1, index=ids_restore)

        return mask.bool()                                        # 对patch按照mask_unit_size进行mask的所用矩阵

    def get_pos_embed(self) -> torch.Tensor:
        if self.sep_pos_embed:
            return self.pos_embed_spatial.repeat(
                1, self.tokens_spatial_shape[0], 1
            ) + torch.repeat_interleave(                # 元素就近重复多次, torch.repeat是整个轴进行阵列 
    ...
    def forward(
        ...
        x = x + self.get_pos_embed()    # 空间维度 + 时间维度的embedding，时间维度的排列顺序形如000111222

        x = self.unroll(x)                                         # 重排列patch, 上下maxpoll2×2的patch的同一个位置排在一起, 能够通过x.view(B, Sx*Sy, -1, C).max(dim=1)实现maxpool

        # Discard masked tokens                                    # mask掉的patch会在这里被丢弃, 每个batch被mask的比例一样
        if mask is not None:
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if return_intermediates and i in self.stage_ends:
                intermediates.append(self.reroll(x, i, mask=mask))  # 记录每一个stage最后的特征图，恢复到原来的位置关系

        if mask is None:                                            # 微调阶段使用
            x = x.mean(dim=1)
            x = self.norm(x)
            x = self.head(x)
